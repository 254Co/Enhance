...
def generate_initial_positions(num_agents):
    """
    Generates initial positions for agents.

    Args:
        num_agents (int): Number of agents.

    Returns:
        list: A list of initial positions.
    """
    return [random.uniform(0, 100) for _ in range(num_agents)]

def generate_historical_prices(num_steps):
    """
    Generates historical prices for initializing the market.

    Args:
        num_steps (int): Number of historical steps.

    Returns:
        list: A list of historical prices.
    """
    return [100 + random.uniform(-1, 1) for _ in range(num_steps)]

def generate_news_sentiment(num_steps):
    """
    Generates news sentiment data.

    Args:
        num_steps (int): Number of steps for news sentiment.

    Returns:
        dict: A dictionary with steps as keys and sentiment values as values.
    """
    return {i: random.uniform(-0.5, 0.5) for i in range(num_steps)}
